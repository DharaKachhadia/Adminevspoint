      ...
      <Table className={classes.table} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell className={classes.tableHeaderCell}>Isuue</TableCell>
        ...
        <TableBody>
          {users.map((row) => (
            ...
            >
              ...
              <TableCell className={classes.name}>{row.DMessage}</TableCell>
              <TableCell><button type="button" className={classes.btn} >Message</button><button className={classes.btn1} type="button" onClick={()=>( fetch(`http://evspoint.com/api/OwnerHelp/${row.Email}`,{
     method: 'DELETE'
   }).then((result) => {
     result.json().then((response) => {
      window.location.href = "helpReq";
     })
