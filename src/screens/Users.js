...
export default function BasicTable() {
  ...
  const fetchData = () => {
 fetch("http://evspoint.com/api/user.json ")
      .then(response => {
  ...
  function DeleteData(contactNo) {
   fetch(`http://localhost:4000/user/${contactNo}`,{
     method: 'DELETE'
   }).then((result) => {
    ...
    <div className="row grid-com">
      ...
      <MdAccountCircle size={120} className="icon-color" />
        ...
        <div className="d-flex justify-content-center align-items-center  mb-3"> 
         <h1 className={classes.h1}>User Informations</h1>
         </div>
        ...
        <TableBody>
          {users.map((row) => (
            ...
            >
              ...
              <TableCell><button type="button" className={classes.btn} >Edit</button><button className={classes.btn1} type="button" onClick={()=>( fetch(`http://evspoint.com/api/user/${row.contactNo}`,{
     method: 'DELETE'
   }).then((result) => {
     result.json().then((response) => {
